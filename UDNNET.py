...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x1 = torch.cat((x.unsqueeze(3), outputs[0].unsqueeze(3), outputs[1].unsqueeze(3), outputs[2].unsqueeze(3),
                        outputs[3].unsqueeze(3)), dim=3)
        x1 = self.skipcat(x1)
        x1 = x1.squeeze(3)
...
class UDNNET(nn.Module):
    def __init__(self, input_channels=102, n_band_1=1584, n_band_2=64, n_band_3=64, num_classes=10,
                 num_tokens=4, dim=64 * 4, depth=4, heads=16, mlp_dim=8, dropout=0.0, emb_dropout=0.0):
        super(UDNNET, self).__init__()
        self.L = num_tokens
        self.cT = dim
...
#
# net = snet(input_channels = 48, num_classes = 7 , dim = 16 * 4)
# inputs = torch.randn(1,48, 9, 9)
