...
parser.add_argument('--halfwidth', default=6, type=int)
# mcc loss
...
seeds = [1241,1098,1801,1747,1901,1142,1595,1496,1519,1976,1840]
...
for iDataSet in range(nDataSet):
    ...
    optimizer_f = optim.Adam(list(F1.parameters()) + list(F2.parameters()), lr=0.01, weight_decay=0.005)
    ...
    def train(ep, train_loader, train_tar_loader):
        ...
        for batch_idx in range(1, num_iter):
            ...
            source_data0 = utils.radiation_noise(data_source, alpha_range=(0.6,1.4) , beta= 1).type(torch.FloatTensor)
            target_data0 = utils.radiation_noise(data_target, alpha_range=(0.6,1.4) , beta= 1).type(torch.FloatTensor)
            ...
            output_t1 = output1[0][m * batch_size:, :]
            output_t2 = output2[0][m * batch_size:, :]
    ...
    for ep in range(1, num_epoch + 1):
        ...
        if value < value1:
            ...
            if value >= best_test_acc:
                # torch.save(G, "checkpoints/pavia/G-{}.pt".format(value))
                # torch.save(F1, "checkpoints/pavia/F1-{}.pt".format(value))
                # torch.save(F2, "checkpoints/pavia/F2-{}.pt".format(value))
                best_test_acc = value
...
# 运行结果分析说明：
# - 每个种子的准确率输出，便于复现实验和横向对比。
# - OA/AA/kappa均值和标准差反映模型整体、类别和一致性表现及其稳定性。
# - 每类准确率均值和标准差有助于分析模型对各类别的适应能力。
# - 训练和测试耗时可用于评估模型效率。
