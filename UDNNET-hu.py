...
parser.add_argument('--temperature', default=4.0,
...
seeds = [1958,1546,1404,1805,1350,1753,1899,1526,1667,1026]
...
for iDataSet in range(nDataSet):
    ...
    def train(ep, train_loader, train_tar_loader):
        ...
        for batch_idx in range(1, num_iter):
            ...
            source_data0 = utils.radiation_noise(data_source, alpha_range=(0.7, 1.3), beta=0.0001).type(torch.FloatTensor)
            target_data0 = utils.radiation_noise(data_target, alpha_range=(0.7, 1.3), beta=0.0001).type(torch.FloatTensor)
            ...
            output_s1 = output1[0][:2 * batch_size, :]
            output_s2 = output2[0][:2 * batch_size, :]
            output_t1 = output1[0][batch_size * 2:, :]
            output_t2 = output2[0][batch_size * 2:, :]
